...
export const getTeachersStudentansAssignment = async (req, res) => {
  try {
    ...
    if (!specializedSubject) {
      return res.status(400).json({ message: "Teacher's specialized subject is missing" });
    }
    ...
    const students = await User.find({
      ...
      subjects: { $in: [specializedSubject] }, // Use teacher's subject
    });
    ...
    res.status(200).json({ message: "Teacher's students fetched and also assignments successfully", students, assignments });
  } catch (error) {
    console.error("Error in getTeachersStudent controller:", error);
    res.status(500).json({ message: "Error fetching teacher's students", error });
  }
};
...
export const getSubmissionofParticularAssignment = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Submitted assignment fetched successfully", submitassignment });
  } catch (error) {
    console.error("Error in getSubmissionofParticularAssignment controller:", error);
    res.status(500).json({ message: "Error fetching submitted assignment", error });
  }
};
...
export const getParticularStudentAssignmentDetails = async (req, res) => {
  try {
    ...
    if (!studentId || !studentClass) {
      return res.status(400).json({ message: "Student ID and class are required." });
    }
    ...
    res.status(200).json({ message: "Particular student's assignment details fetched successfully", submitassignment, student });

  } catch (error) {
    console.error("Error in getParticularStudentAssignmentDetails controller:", error);
    res.status(500).json({ message: "Error fetching particular student details", error });
  }
};
...
export const getSubmittedAssignments = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Submitted assignments fetched successfully", viewSubmittedAssignments });
  } catch (error) {
...
export const updateSubmission = async (req, res) => {
  try {
    ...
    const updateSubmission = await Submission.findByIdAndUpdate(editSubmittedAssignmentId, {aiMarks, aiShortFeedback}, {new: true});
    res.status(200).json({ message: "Submission updated successfully", updateSubmission });
  } catch (error) {
